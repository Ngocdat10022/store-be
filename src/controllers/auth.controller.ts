...
import { UserRegister, UserLogin } from '@interfaces/users.interface';
...
export class AuthController {
    ...
    try {
      ...
      db.query(getUserEmail,[userData.email.trim()], async (err: Error, data: any)=>{
        ...
        if(data.length){
          ...
          const value = [userData.firstName,userData.lastName, userData.email, hashedPassword];
          db.query(insertUser, [value], (err:Error, data:any) => {
            if (err) return res.status(409).json(err);
  ...
  public logIn = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
    try {
      ...
      db.query(getUserEmail, [userData.email.trim()], (err:Error, data:any) => {
        if (err) return res.status(409).json(err);
